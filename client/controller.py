...
class ConnectionService(TCPConnection):
    """Service to communicate with the game server after logging in. Uses a TCP connection."""


    def __init__(self, address: tuple[str, int], client_game_state: ClientGameState, dispatcher: UIEventDispatcher):
        ...
        try:
            match packet.tag:
                ...
                case PacketTag.NEW_BOSS:
                    typed_packet = self.get_typed_packet(packet, BossData)
                ...
                case PacketTag.BOSS_DEAD:
                    typed_packet = self.get(packet, StringMessage)
...
class GameController:
    ...
    def on_logged_in(self, username: str, address: tuple[str, int], game_state: PlayerGameStateData):
        ...
        self._connection_service = ConnectionService(address, self.client_game_state, self.dispatcher)
